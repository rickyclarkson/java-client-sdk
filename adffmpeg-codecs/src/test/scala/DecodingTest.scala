...
import uk.org.netvu.jpeg.{JPEGDecoder, JPEGDecoderFromArray}

class DecodingTest extends JUnit4(new Specification {
 for (decoder <- List[JPEGDecoder with JPEGDecoderFromArray](ADFFMPEGDecoder.getInstance()))
  decoder.toString + isSpecifiedBy(ValidDecoder(decoder))
})

case class ValidDecoder(decoder: JPEGDecoder with JPEGDecoderFromArray) extends Specification {
 for (file <- List((352, 256, "../jpeg/192-168-106-206-352x256.jpg" ),
