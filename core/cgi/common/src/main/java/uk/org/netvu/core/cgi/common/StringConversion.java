...
/**
 * A StringConversion can convert between Strings and a given type, T, though the conversion can be partial, which is represented by an Option in the results. A partial
...
public final class StringConversion<T>
{
    ...
    private StringConversion(Conversion<String, Option<T>> conversionFromString, Conversion<T, Option<String>> conversionToString)
    {
    ...
    /**
     * Converts Strings to Ts, returning the result in an Option that contains a T if the conversion succeeds, and nothing if it fails.
     */
    public Option<T> fromString(String string)
    {
        return conversionFromString.convert(string);
    }

    /**
     * Converts Ts to Strings, returning the result in an Option that contains a String if the conversion succeeds, and nothing if it fails.
     */
    ...
    /**
     * Constructs a StringConversion from a partial Conversion from Strings to Ts, and a partial Conversion from Ts to Strings.
     ...
     * @param <T> the type that this StringConversion can convert Strings to and from.
     * @return a StringConversion that can convert Strings to Ts and Ts to Strings.
     */
    public static <T> StringConversion<T> partial(final Conversion<String, Option<T>> conversionFromString,
    ...
    /**
     * Constructs a StringConversion from a partial Conversion from Strings to Ts, and T's toString() method.
     * @param conversionFromString the partial Conversion from Strings to Ts.
     * @param <T> the type that this StringConversion can convert Strings to and from.
     * @return a StringConversion that can convert Strings to Ts and Ts to Strings.
     */
    public static <T> StringConversion<T> convenientPartial(final Conversion<String, Option<T>> conversionFromString)
    {
        return partial(conversionFromString, andThenSome(Conversion.<T>objectToString()));
    }

    /**
     * Constructs a StringConversion from a Conversion from Strings to Ts, and T's toString() method.
     * @param conversionFromString the Conversion from Strings to Ts.
     * @param <T> the type that this StringConversion can convert Strings to and from.
     * @return a StringConversion that can convert Strings to Ts and Ts to Strings.
     */
    public static <T> StringConversion<T> convenientTotal(final Conversion<String, T> conversionFromString)
    {
        return partial(andThenSome(conversionFromString), andThenSome(Conversion.<T>objectToString()));
    }

    /**
     * Constructs a StringConversion from a Conversion from Strings to Ts, and a Conversion from Ts to Strings.
     * @param conversionFromString the Conversion from Strings to Ts.
     * @param conversionToString the Conversion from Ts to Strings.
     * @param <T> the type that this StringConversion can convert Strings to and from.
     * @return a StringConversion that can convert Strings to Ts and Ts to Strings.
     */
    public static <T> StringConversion<T> total(final Conversion<String, T> conversionFromString,
