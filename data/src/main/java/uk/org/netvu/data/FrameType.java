...
enum FrameType
{
    ...
    private static JFIFPacket jpegToJfif( ByteBuffer source, StreamMetadata metadata, ImageData imageData )
    {
        ...
        commentOriginal.limit(imageData.startOffset);
        ByteBuffer comment = getComment(imageData, commentOriginal);
        byte[] yqFactors = getYQFactors(imageData.qFactor);
        byte[] uvqFactors = getUVQFactors(imageData.qFactor);
        ...
        sof.putShort(imageData.format.targetLines);
        sof.putShort(imageData.format.targetPixels);
        sof.order(ByteOrder.BIG_ENDIAN);
        sof.put(byteArrayLiteral(new int[]{ 0x03, 0x01, imageData.videoFormat == VideoFormat.JPEG_411 ? 0x22 : 0x21, 0x00, 0x02,
        ...
        ByteBuffer jfif = ByteBuffer.allocate( JFIF_HEADER.length + SOC_HEADER.length + 2 + comment.limit() + YQ_HEADER.length + yqFactors.length + UVQ_HEADER.length + uvqFactors.length + sof.limit() +
                                               HUFFMAN_HEADER.length + SOS_HEADER.length + source.limit() - imageData.startOffset + EOI_MARKER.length );
        ...
        jfif.put(SOS_HEADER);
        source.position(imageData.startOffset);
        jfif.put(source);
        jfif.put(EOI_MARKER);
        jfif.position(0);
    ...
    {
        int bufferCapacity = getCommentByteCount(imageData.camera, imageData.utcOffset, commentData) + imageData.title.length() + imageData.alarm.length() + "dd/MM/yyyy".length() + "HH:mm:ss".length() + 256;
        ByteBuffer buffer = ByteBuffer.allocate(bufferCapacity);
        println(buffer, "Version: 00.03");
        println(buffer, "Number: " + imageData.camera);
        println(buffer, "Name: " + imageData.title);
        Date date = new Date(imageData.sessionTime * 1000L);
        println(buffer, "Date: " + dateFormatter.format(date));
        println(buffer, "Time: " + timeFormatter.format(date));
        println(buffer, "MSec: " + imageData.milliseconds % 1000);
        println(buffer, "Locale: " + imageData.locale);
        println(buffer, "UTCoffset: " + imageData.utcOffset);
        if (imageData.alarm.length() > 0)
        {
            println(buffer, "Alarm-text: " + imageData.alarm);
        }
