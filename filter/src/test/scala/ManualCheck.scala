...
object ManualCheck { def main(args: Array[String]) = {
 ...
 val filters: List[(ImageFilter, String)] = List(new ImageFilter { def filter(image: Image) = image } -> "No filter",
                                                 ...
                                                 Filters.andThen(ContrastFilter.contrastFilter(1.5), BrightnessFilter.betterBrightnessFilter(30)) -> "Contrast filter then brightness filter",
                                                 Filters.andThen(MonochromeFilter.monochromeFilter(0.5, 0.2, 0.3), BrightnessFilter.betterBrightnessFilter(30)) -> "Monochrome filter then brightness filter")
