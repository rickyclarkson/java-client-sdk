...
//parameterise the comparisons so that jpeg/ calls it with ImageIODecoder and adffmpeg-codecs with ADFFMPEGDecoder
class DecodingTest extends JUnit4(new Specification {
 ...
 for (file <- SampleData.data) {
  def decodeToIntArray(decoder: JPEGDecoder): Array[Int] = { val image = ToolkitDecoder.createInstance decodeJPEG Buffers.bufferFor(file.name)
  ...
  val fromImageIO = decodeToIntArray(ImageIODecoder.createInstance)
  ...
  def equal: Boolean = {
   for { i <- 0 until control.length
         ...
         if abs(r2 - r) > 5
         if abs(g2 - g) > 5
         if abs(b2 - b) > 5 } return false
   return true
  }
  
  "An image decoded by ImageIO" should { "be roughly equal to an image decoded by Toolkit" in {
   equal must beTrue
  } }
 }
})
