...
object proxymode { def main(args: Array[String]) = {
 print(lines("package uk.org.netvu.protocol;") ++ blankLine ++ 
       Enum("ProxyMode", "This controls whether or not a decoder that is connected to by the server maintains connections to cameras set up by the CGI request", IsStatic(false),
