...
object replaypiccgi { def main(args: Array[String]): Unit = {  
  val params = List(
   ...
   ParaMeta("Control", "CONTROL", lines("""ParameterDescription.parameter( "control", StringConversion.convenientPartial( Control.fromStringFunction() ) ).withDefault( Control.STOP )"""), "Control", "getControl", "control"),
   ...
   ParaMeta("OnOrOff", "AUDIO", lines("""ParameterDescription.parameter( "audio", StringConversion.convenientPartial( OnOrOff.fromStringFunction() ) ).withDefault( OnOrOff.OFF )"""), "OnOrOff", "isAudioOn", "audioOn"),
 ...
 lines("public static Function<String, Option<Integer>> fromTimeFunction()") ++ brace(append(prepend("return ", function("String", "Option<Integer>", "s", lines(
